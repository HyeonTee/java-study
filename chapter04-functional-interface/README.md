...
이 wrap-and-rethrow 패턴은 Phase 2의 `CompletableFuture` 예외 전파(ch10)에서 다시 만난다. 검사 예외를 비검사로 바꾸면 원래 예외를 `cause`로 보존하는 것이 중요하다.
...
검사 예외를 던지는 함수를 표준 `Function`으로 변환. ch10 예외 전파의 선수 패턴.
