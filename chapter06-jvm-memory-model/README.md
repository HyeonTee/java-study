# Chapter 06 — JVM 메모리 모델

> **선행 단원**: Phase 1(Chapter 01~05)의 Java 문법과 표준 API를 익힌 상태에서 진행한다. 특히 Chapter 01에서 다룬 `new`를 통한 객체 생성, Chapter 05의 `record`가 힙에 어떻게 저장되는지를 이 단원에서 JVM 관점으로 다시 본다.
...
대신 `VisibilityDemo`를 **직접 실행해보며 관찰**한다. 동시성을 제대로 *구현*하고 검증하는 일(올바른 게시, 동기화)은 Phase 2의 ch09(thread-basics) 이후에서 적절한 도구로 다룬다. 이 단원은 "왜 그런 보장이 필요한가"라는 **메모리 모델의 이유**에 집중한다.
