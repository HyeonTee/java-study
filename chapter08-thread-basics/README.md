# Chapter 08 — 스레드 기초 (Thread, synchronized, volatile, Atomic)
...
ch06은 "보장이 없으면 위험하다"는 **이유**에서 끝났다. ch08은 그 위에서 다음 흐름을 따른다:
...
이 단원에서 다루지 않는 것(경계):
- **고수준 도구** — `ExecutorService`, 스레드풀, `BlockingQueue`, Producer-Consumer, `wait`/`notify`, `ReentrantLock`은 **ch09**에서. ch08은 날것의 `Thread`로 먼저 근육을 만든다.
...
./gradlew :chapter08-thread-basics:test

# 특정 클래스
./gradlew :chapter08-thread-basics:test --tests "study.chapter08.SafeCounterTest"
```
