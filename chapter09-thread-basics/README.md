# Chapter 09 — 스레드 기초 (Thread, synchronized, volatile, Atomic)

> **선행 단원**: Chapter 06(JVM 메모리 모델). ch06은 **왜** 멀티스레드에서 값이 안 보이고 순서가 뒤집히는지(가시성·재배치·happens-before)를 **이론과 데모로** 설명했다. 이 단원은 그 약속을 이행한다 — ch06이 "데모로만 보여주고 미뤄둔" **올바른 게시(publication)와 동기화를 직접 *구현*하고, 결정적으로 *검증*한다.**
...
ch06은 "보장이 없으면 위험하다"는 **이유**에서 끝났다. ch09은 그 위에서 다음 흐름을 따른다:
...
이 단원에서 다루지 않는 것(경계):
- **고수준 도구** — `ExecutorService`, 스레드풀, `BlockingQueue`, Producer-Consumer, `wait`/`notify`, `ReentrantLock`은 **ch10**에서. ch09은 날것의 `Thread`로 먼저 근육을 만든다.
- **`Thread.start()`/`join()`의 happens-before 이론** — ch06에서 이미 다뤘다. 여기서는 그 규칙에 *의존하는 코드를 짠다*.
...
한 스레드가 변수를 바꿔도, 다른 스레드는 자기 CPU 캐시에 든 **옛 값**을 계속 볼 수 있다(ch06 "Working Memory" 그림). 대표 증상은 **멈추지 않는 루프**다:
...
- 같은 모니터에 대해 **unlock → lock**은 happens-before 관계다(ch06 규칙 2). 그래서 블록을 나갈 때 쓴 값이 다음에 들어오는 스레드에게 **보장되어 보인다** → 가시성도 해결.
...
2. **재배치 제한** — `volatile` 쓰기 이전의 명령이 그 뒤로 재배치되지 않는다(ch06 규칙 1).
...
ch06의 원칙을 그대로 따른다: **잘못된 구현이 초록불이 되거나, 올바른 구현이 빨간불이 되는(flaky) 테스트는 학습에 해롭다.** 동시성은 비결정적이라 이게 진짜 어렵다. 핵심 통찰:
...
- **정지 플래그(채점은 "멈춘다"만)**: `@Timeout`으로 "정지 신호를 받으면 종료한다"는 **기능**을 채점한다. "`volatile`이 *없으면* 안 멈춘다"는 x86에서 재현이 보장되지 않으므로 채점하지 않고, ch06의 `VisibilityDemo`(실행 관찰)로 미룬다.
- **레이스 *재현*(데모)**: "비동기화 카운터가 값을 잃는다"는 비결정적이라 단언하면 그 자체가 flaky다. 그래서 ch06의 GC 패턴처럼 **손실이 관측된 경우에만 단언하고, 아니면 `assumeTrue`로 skip**한다(`UnsafeCounterDemoTest`).
...
./gradlew :chapter09-thread-basics:test

# 특정 클래스
./gradlew :chapter09-thread-basics:test --tests "study.chapter09.SafeCounterTest"
```
