# Chapter 14 — IO 기초 (스트림 · 데코레이터 · 프레이밍 · NIO ByteBuffer · try-with-resources)

> **선행 단원**: Chapter 04(검사 예외 wrap-and-rethrow — `IOException`), Chapter 01(배열·버퍼 감각). **Phase 2 블록의 마지막 단원**이자 **Phase 3(네트워크)의 토대**: 여기서 깎는 **라인 프레이밍은 ch15 소켓 프로토콜**, **길이 프레이밍은 ch16 HTTP `Content-Length`**, **try-with-resources는 ch15 소켓 수명주기**로 그대로 이어진다.
...
파일·소켓 없이 **인메모리 바이트 배열**(`ByteArrayInputStream`/`ByteArrayOutputStream`)로 그 파이프를 직접 만든다 — 소켓이 없을 뿐 다루는 바이트 파이프라인은 ch15와 동일하다. 전부 인메모리라 **완전 결정적**이다(동시성 단원과 달리 타이밍·flaky 없음).

```
바이트 계약(read()=int 0~255, EOF=-1)
  → copy 루프 → 데코레이터(척추) → 프레이밍(라인/길이) → ByteBuffer → try-with-resources
                                       │                │                     │
                                   ch15 소켓        ch16 HTTP             ch15 소켓 close
```

다루지 않는 것(경계): 실제 파일시스템(`Files`/`Path`), Selector·논블로킹 IO(→ ch15/ch17), 객체 직렬화(`Serializable`), 압축(GZIP), mmap.
...
- **라인 프레이밍**(`LineReader`, → ch15): 구분자 `\n`(앞의 `\r`은 CRLF로 제거)까지 모아 한 줄. 단독 `\r`은 구분자 아님. 마지막 줄은 종결자 없이 EOF로 끝나도 반환. 빈 줄("")과 끝(null)을 구분.
- **길이 프리픽스 프레이밍**(`FrameCodec`, → ch16): 먼저 **4바이트 빅엔디안 int 길이**를 쓰고 그만큼 페이로드. `Content-Length`가 정확히 이것이다.
...
`Resources.closeAll(...)`로 이 LIFO·suppressed 로직을 직접 구현한다. ch15 소켓 수명주기가 이 위에 선다.

> ch04와의 선: ch04는 "검사 예외를 비검사로 wrap하는 어댑터"가 주제. ch14는 "검사 예외를 try-with-resources/suppressed로 다루는 자원 수명"이 주제다.
...
### LineReader (3문제) — 라인 프레이밍 · ch15 직결
생성자/`readLine()`(null 종료)/`readLines()`. LF·CRLF·빈 줄·UTF-8·마지막 줄.

### FrameCodec (2문제) — 길이 프레이밍 · ch16 직결
...
./gradlew :chapter14-io-basics:test

# 특정 클래스
./gradlew :chapter14-io-basics:test --tests "study.chapter14.LineReaderTest"
```
