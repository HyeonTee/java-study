...
### 규칙 5: final 필드 — 안전 초기화(safe publication)

객체의 `final` 필드는 **생성자가 끝나는 시점**에 그 값이 확정되고, 생성자가 끝난 뒤 다른 스레드가 그 객체를 보면 `final` 필드는 **반드시 초기화된 값으로 보인다** — 동기화 없이도. (단 생성자 안에서 `this`를 밖으로 새게 하면(this-escape) 이 보장이 깨진다.)
...
// 한 스레드가 new Point(1,2)를 만들어 공유하면, 다른 스레드는 x=1, y=2를
// 절대 0(기본값)으로 보지 않는다 — final이 주는 happens-before 보장.
```

이것이 **불변 객체(immutable)가 스레드 안전한 이유**의 절반이다(나머지 절반은 "한 번 쓰고 안 바꿈"). ch09에서 `final` + 방어적 복사로 불변 객체를 만들 때 이 규칙이 바탕이 된다.
...
| 프로그램 순서 | 같은 스레드의 앞 코드 | 같은 스레드의 뒷 코드 | 단일 스레드의 **결과**는 순차 실행과 동일(as-if-serial) |

> "프로그램 순서"는 명령이 **반드시 그 순서로 실행**된다는 뜻이 아니다. JVM/CPU는 단일 스레드 안에서도 명령을 **재배치(reordering)**할 수 있다 — 다만 그 스레드가 **혼자 봤을 때의 결과**가 순차 실행과 같도록 보장될 뿐이다(**as-if-serial**). 위 "재배치 문제" 절과 모순이 아니라, 재배치는 *다른 스레드*에서만 관측된다는 뜻이다.
...
> **`ReferenceQueue` — "수거됐다"는 통지**: `Soft`/`Weak`/`PhantomReference`를 만들 때 `ReferenceQueue`를 함께 등록하면, GC가 그 대상을 수거할 때(정확히는 수거 단계에 진입할 때) 해당 `Reference` 객체가 큐에 **등록(enqueue)**된다. 큐를 폴링하면 "어떤 객체가 비워졌는지"를 알 수 있어 연관 자원을 정리한다(예: `WeakHashMap`이 죽은 엔트리를 청소하는 방식). 특히 **`PhantomReference.get()`은 언제나 `null`**을 반환한다 — 객체를 되살릴 수 없게 막은 채 "수거 직전" 시점만 큐로 통지받는 용도라, `finalize()`(되살리기 가능·시점 불명)의 안전한 대체다.
...
대신 `VisibilityDemo`를 **직접 실행해보며 관찰**한다. 동시성을 제대로 *구현*하고 검증하는 일(올바른 게시, 동기화)은 Phase 2의 ch11(thread-basics) 이후에서 적절한 도구로 다룬다. 이 단원은 "왜 그런 보장이 필요한가"라는 **메모리 모델의 이유**에 집중한다.
