# Chapter 13 — ExecutorService · 스레드풀 · BlockingQueue · Producer-Consumer
...
ch11에서 `count++`가 원자성·가시성·CAS를 관통했듯, ch13은 **`take()`/`put()`의 조건 대기** 하나가 wait/notify·Condition·Producer-Consumer·스레드풀을 전부 꿴다.
...
이 단원에서 다루지 않는 것(경계): **`CompletableFuture`·비동기 파이프라인 → ch14**, **Virtual Thread → ch15**, `Semaphore`/`CountDownLatch`/`CyclicBarrier`(동기화 보조 도구는 *사용*만, 주제 아님), `ForkJoinPool`/work-stealing, `ConcurrentHashMap` 등 동시 컬렉션. `Future`/`Callable`로 결과를 회수하는 것도 ch14로 미룬다 — 여기 스레드풀은 `Runnable`만 다룬다.
...
./gradlew :chapter13-executor-blocking-queue:test

# 특정 클래스
./gradlew :chapter13-executor-blocking-queue:test --tests "study.chapter13.MyBlockingQueueTest"
```
