# Chapter 14 — Virtual Thread · Structured Concurrency

> **선행 단원**: Chapter 11(Thread)·11(스레드풀/BlockingQueue)·12(CompletableFuture). Phase 2(동시성)의 마지막 단원. ch12이 "스레드는 비싸니 풀로 재사용"이라는 전제 위에 풀을 쌓았다면, 이 단원은 그 전제를 **무너뜨린다**(가상 스레드는 싸다 → 풀링하지 마라). 다음 Phase 3의 **ch18(concurrent-http-server)이 "가상 스레드/연결" 모델을 재사용**한다.
...
ch12  "비싸니 풀로 재사용" — MyThreadPool: 고정 워커 N + 작업 큐
        ↓  전제를 뒤집는다
ch14  가상 스레드: 스레드가 싸다 → 풀 버리고 작업마다 새로 만든다 (thread-per-task)
        ↓  흩어진 작업의 수명을 다시 묶는다
      structured concurrency: fork → 전부 join → 첫 실패 시 형제 취소·전파
        ↓
ch18  concurrent-http-server: 연결마다 가상 스레드
```
...
- **풀링하지 마라**: 생성이 공짜이므로 재사용할 이유가 없다. `Executors.newVirtualThreadPerTaskExecutor()`는 **작업마다 새 스레드**를 만든다(ch12 고정 풀과 정반대). 동시성 제한이 필요하면 풀이 아니라 `Semaphore`.
...
> **pinning 함정**: 가상 스레드가 `synchronized` 블록 안에서 **블로킹**하면 캐리어에 **고정(pinned)**되어 언마운트 못 한다 → 경량성 붕괴. 블로킹 구간을 감싸는 `synchronized`는 ch12의 `ReentrantLock`으로 바꾼다. (Java 21 기준 — 짧은 비블로킹 임계구역은 무해.)
...
> 표준 `java.util.concurrent.StructuredTaskScope`는 Java 21 **프리뷰**다 — `--enable-preview`가 필요하고 버전마다 시그니처가 바뀐다(빌드 취약). 그래서 이 단원은 그것을 쓰지 않고, **핵심 의미론을 가상 스레드 위에 `MyTaskScope`로 직접 구현**한다(ch12 `MyThreadPool`, ch13 `MyPromise`와 같은 "내부 직접 구현" 방식). 실무에선 표준 API를 쓰되 그 내부가 이렇게 생겼음을 안다.
...
ch13 `MyPromise`(값 완료 | 예외 완료, 단 한 번)의 모델을 작업 단위로, ch13 `allOf`(전부 대기)에 **취소·수명 묶기**를 더한 것이다.
...
./gradlew :chapter14-virtual-thread:test

# 특정 클래스
./gradlew :chapter14-virtual-thread:test --tests "study.chapter14.MyTaskScopeTest"
```
