 ...
 * 스레드 위에 손으로 만든다(ch13 {@code MyThreadPool}, ch14 {@code MyPromise}처럼 "표준 내부를 직접 구현").
 *
 * <p><strong>구조적 동시성</strong>이란: 한 블록 안에서 띄운 모든 하위 작업의 수명이 그 블록을 벗어나지
 * 않게 묶는 것이다. ch13 스레드풀의 작업들은 서로 무관했지만, 여기서는 {@link #fork}로 띄운 작업들을
 * 하나의 단위로 보고 {@link #join}에서 한꺼번에 기다린다(ch14 {@code allOf}의 "수명 묶기" 버전).
 *
...
public class MyTaskScope implements AutoCloseable {
    ...
    /**
     * {@link #fork}로 띄운 하나의 하위 작업 핸들. ch14 {@code MyPromise}의 축소판 — 상태는
