 ...
 *
 * <p>ch13에서는 스레드가 비싸서 <strong>풀링</strong>으로 N개를 재사용했다. 가상 스레드는 OS 스레드가
 * 아니라 JVM이 스케줄링하는 경량 스레드라, 수만 개를 만들어도 괜찮다 — 그래서 풀을 버리고 <strong>작업마다
 * 새 스레드</strong>(thread-per-task)로 돌아간다. API는 ch11의 {@code Thread}/{@code join}과 <strong>똑같고</strong>,
...
public final class VirtualThreads {
     ...
     * (start 따로 / join 따로 — ch11 ParallelSum 복습), 전부 {@code join()}하라. {@code join()}의
