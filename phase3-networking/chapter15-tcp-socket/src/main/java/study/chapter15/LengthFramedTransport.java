 ...
 * <strong>ch14 {@code FrameCodec} → ch16 HTTP {@code Content-Length}</strong>의 다리.
 *
 * <p>와이어 포맷(big-endian, 네트워크 바이트 순서): <pre>[ 4바이트 int 길이 N ][ N바이트 페이로드 ]</pre>
 *
 * <p><strong>ch14와의 차이(정직한 가교)</strong>: ch14 {@code FrameCodec}은 {@code ByteBuffer}(이미 완성된 버퍼)를
 * 가정했다. 그러나 실제 소켓엔 "완성된 버퍼"가 없다 — 바이트가 찔끔찔끔 도착한다. 그래서 ch15의 새 살은
 * <strong>"정확히 N바이트가 모일 때까지 채워 읽는 루프"</strong>({@link #readFully})다. 이것이 길이 프레이밍의
 * 본질이자 ch14 copy 루프의 응용이다.
 *
...
public final class LengthFramedTransport {
     ...
     * 시프트 연산) 페이로드를 잇는다. ch14 {@code FrameCodec.encode(payload)}로 바이트를 만든 뒤 그대로 흘려도 된다.
     *
