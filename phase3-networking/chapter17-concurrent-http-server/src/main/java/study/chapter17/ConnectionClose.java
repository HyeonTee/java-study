...
/**
 * <strong>connection-close(EOF까지) 프레이밍</strong> — ch16 {@code parseResponse}가 명시적으로 ch17로 미룬 네 번째 모드.
 *
...
public final class ConnectionClose {
    ...
    /**
     * <strong>응답</strong> 바디의 프레이밍을 헤더로 결정해 읽는다 — ch16 세 모드 + connection-close 네 번째 모드의 합류점.
     *
     * <p>힌트: ch16 {@code BodyFraming.from(headers)}를 {@code switch}(pattern matching)로 분기 —
     * {@code ContentLength(len)}이면 ch16 {@code HttpParser.readFully}, {@code Chunked}면 ch16
