 ...
 * 코드로 증명하는 자리.
 *
 * <p>두 개의 원시 빌딩블록이 ch16에서 그대로 온다:
 * <ul>
 *   <li>{@link #readLine}(InputStream) — ch16 {@code LineProtocol.readLine}과 <strong>같은 계약</strong>(인스턴스→static).
 *       시작줄·헤더의 <strong>라인 프레이밍</strong>.</li>
 *   <li>{@link #readFully}(InputStream,int) — ch16 {@code LengthFramedTransport.readFully}와 같은 계약. 바디의
 *       <strong>길이 프레이밍</strong>(= Content-Length).</li>
 * </ul>
 *
 * <p>{@code Socket}을 import하지 않는다(ch16 {@code ConnectionHandler} 경계 계승) — 그래서 {@code ByteArrayInputStream}으로
 * 100% 결정적으로 테스트된다.
 */
public final class HttpParser {
    ...
    /**
     * ch16 {@code LineProtocol.readLine}과 글자 그대로 같은 계약: 한 바이트씩 {@code '\n'}까지 누적, 끝 {@code '\r'}
    ...
    public static String readLine(InputStream in) throws IOException {
        throw new UnsupportedOperationException(
                "TODO: ch16 LineProtocol.readLine과 동일 계약(인스턴스→static). 반드시 read() 한 바이트씩 — read(buf)는 헤더 경계 넘어 바디를 삼킨다");
    }

    /**
     * ch16 {@code LengthFramedTransport.readFully}와 같은 계약: 정확히 {@code n}바이트를 채워 읽어 반환. 부분 읽기를
    ...
    public static byte[] readFully(InputStream in, int n) throws IOException {
        throw new UnsupportedOperationException(
                "TODO: ch16 readFully와 동일 — off가 n이 될 때까지 read(b,off,n-off) 루프. 즉시 EOF=null, 도중 EOF=EOFException");
    }
     ...
     * 없으면 EOF까지 읽기" = connection-close 프레이밍은 <strong>ch18 경계</strong>이므로 여기서 구현하지 않는다.
     *
     * @throws IOException I/O 오류 또는 {@link HttpProtocolException}(malformed)
     */
    public static HttpResponse parseResponse(InputStream in) throws IOException {
        throw new UnsupportedOperationException(
                "TODO: 상태줄 → 헤더 → BodyFraming 분기(ContentLength/Chunked/None). connection-close(EOF까지)는 ch18 — 여기선 None=빈 바디");
    }
}
