...
/**
 * 라인 단위 request/response 송수신의 코어 — 이 단원의 간판이자 <strong>ch16 {@code LineReader}의 직계</strong>.
 *
 ...
 * 그래서 ch16가 {@code ByteArrayInputStream}으로 라인 프레이밍을 결정적으로 검증했듯, 소켓 위 프로토콜도
 ...
 * 라인 프레이밍({@code \n} 기준 절단)이 그 경계를 복원한다.
 *
 * <p><strong>읽기 계약</strong>(ch16 {@code LineReader} 그대로):
 * <ul>
 ...
 * 쓰는 네트워크 표준 종결자이며, ch18 HTTP로 그대로 이어진다. 읽기는 LF·CRLF 모두 흡수하므로 라운드트립이 성립한다.
 */
public final class LineProtocol {
    ...
    public LineProtocol(InputStream in, OutputStream out) {
        throw new UnsupportedOperationException(
                "TODO: in/out null 검증 후 보관. 소켓은 모른다 — 스트림만. (ch16 LineReader처럼 in을 라인 프레이밍에 쓴다)");
    }
     ...
     * 모은 바이트는 UTF-8로 디코딩하라. (ch16 {@code LineReader.readLine()}과 동일한 계약 — 풀이를 재사용해도 좋다.)
     *
