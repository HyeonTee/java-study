# Chapter 18 — 동시성 HTTP 서버 (keep-alive 연결 루프 × 교체 가능한 Executor · DoS 방어 · 4 스레딩 모델)

> **선행 단원**: Chapter 17(http-protocol — `HttpParser.parse`/`parseResponse`, `HttpMessageWriter`, `Headers`, `BodyFraming`, `HttpProtocolException`, `record` 메시지 모델), Chapter 16(tcp-socket — `ServerSocket(:0)`·`ConnectionHandler` 경계·half-close·4규율), Chapter 14(가상 스레드), Chapter 12(스레드풀), Chapter 11(연결당 스레드), Chapter 08(`volatile` 가시성). **Phase 3의 세 번째 단원** — ch17이 만든 **1왕복 핸들러**를 `accept` 루프 + keep-alive 연결 루프 + 교체 가능한 동시성에 꽂는다.
...
> ⚠️ **이 단원은 Gradle 의존을 갖는 유일한 단원**이다 — `build.gradle`에 `implementation project(':chapter17-http-protocol')`. ch17의 파서/직렬화기를 **복사하지 않고 실제 의존으로 끌어온다.** 그래서 **`main` 브랜치에선 ch17·ch18 스텁이 둘 다라 ch18 테스트가 전부 빨간불**이다(정상). `solve`에서 ch17까지 푼 뒤에야 초록불이 된다.
...
`HttpServer`는 **스레딩 모델을 모른다**(연결을 `executor.execute`에 넘기는 한 줄뿐). `ConnectionLoop`는 **소켓을 모른다**(`InputStream`/`OutputStream`만 안다 — ch16 `ConnectionHandler` 정신). 이 두 "모름"이 직교성의 물리적 구현이다.
...
**다루지 않는 것(경계)**: 라우팅(@Route 리플렉션)·미들웨어 체인·JSON 직렬화 → **ch19**. backpressure/큐 한계·graceful drain의 정밀 제어·HTTP/2 멀티플렉싱·TLS·WebSocket·진짜 부하 벤치마크 → 이론 박스로만.

---

## keep-alive 연결 루프 — 이 단원의 무게중심 (`ConnectionLoop.serve`)

ch17은 한 소켓에 **1왕복**만 했다(요청 1 → 응답 1 → close). keep-alive는 **한 소켓에 요청 N개**를 문다:
...
loop:
  ...
  writeResponse(out, res)                ← ch17 직렬화기 그대로
  count++
...
> 📦 **EOF 계약 차이 — ch17 `parse`는 예외, ch18 `parseOrNull`은 `null`**
> ch17 `HttpParser.parse`는 즉시 EOF에서 **예외**를 던졌다("요청이 와야 한다" — 1왕복 계약). 그러나 keep-alive 루프는 정반대가 필요하다: 요청 사이의 **조용한 EOF는 정상 종료**(클라가 더 안 보냄)다. 그래서 `BoundedHttpParser.parseOrNull`은 요청 전 EOF에서 **`null`**을 반환한다. **ch17 `parse`를 keep-alive 루프에 그대로 쓰면 안 된다** — idle 종료마다 예외가 터진다. 이 계약 한 줄 차이가 N왕복 서버의 핵심이다.
...
ch17에서 떡밥만 깔았던 **세 번째(가 아니라 0번째) 프레이밍**: `Content-Length`도 `chunked`도 없으면 **연결을 끊는 것이 곧 메시지 끝**(EOF = 프레임 경계)이다.
...
> 공격자는 헤더를 1바이트씩 영원히 보내거나(slowloris), 8GB짜리 요청라인 한 줄을 보낸다. **다 읽은 다음 길이를 검사하면 이미 OOM**이다. 그래서 `readLineBounded`는 **누적 바이트가 `maxBytes`를 넘는 순간 즉시** `RequestLimitException`을 던진다(끝까지 안 읽음). idle 소켓은 `soTimeout`(ch16 회수)으로 끊는다.
...
| **고정 스레드풀** | `Executors.newFixedThreadPool(n)` | ch12 | 워커 재사용·동시성 상한 | 블로킹 IO에서 모든 워커가 keep-alive에 묶이면 **풀 고갈**(큐만 쌓임) |
| **연결당 가상 스레드** | `newVirtualThreadPerTaskExecutor()` | ch14 | IO 바운드에 최적·풀 고갈 없음 | (CPU 바운드엔 이득 없음) |
...
> - **`HttpServerIntegrationTest`**(진짜 소켓): 동시성에 대해 **소수의 강한 불변식만** — N개 동시 클라가 `CountDownLatch` 출발 배리어로 한꺼번에 출발 → **응답 바디 집합 == 보낸 target 집합**(뒤섞임·유실 0). 순서·타이밍은 안 본다. `ServerSocket(:0)` + `@Timeout` + 데몬 스레드(ch16 4규율).
>
...
1. **정지 플래그는 `volatile`** — accept 스레드와 `close` 스레드가 다른 스레드라 가시성 없이는 `closed` 변경이 안 보여 영원히 accept(ch08 회수). 닫힌 `ServerSocket`의 `accept()`는 `SocketException`으로 깨어나므로 그것을 종료 신호로(ch16 회수).
2. **한 연결의 예외는 그 연결만 격리** — `handleConnection`이 던지면 소켓만 닫고 다음 연결·서버 전체엔 영향 0(ch12 "작업 하나 실패가 풀을 안 죽임").
...
### 완성 제공(채우지 않음)
`HttpHandler`(@FunctionalInterface 경계), `RequestLimitException`(ch17 예외 상속·431/413), `ServerLimits`(record·`defaults()`), `Responses`(ok/text/error·`Content-Length` 자동 확정). 테스트 픽스처 `ChunkedInputStream`(ch16 복사), `KeepAliveClient`(half-close 없는 연속 exchange).
...
```sh
# 전체 테스트 — main에선 ch17·ch18 스텁이 둘 다라 전부 빨간불(정상). solve에서 ch17+ch18을 풀면 초록불.
./gradlew :chapter18-concurrent-http-server:test

# 무게중심만(in-memory, 결정적)
./gradlew :chapter18-concurrent-http-server:test --tests "study.chapter18.ConnectionLoopTest"

# 진짜 소켓 + 동시성 불변식
./gradlew :chapter18-concurrent-http-server:test --tests "study.chapter18.HttpServerIntegrationTest"
```

---

## 다음 단원(ch19 mini-web-framework)으로

이 단원이 **주입받는 자리**(`HttpHandler`)에, ch19은 **@Route 리플렉션 라우터**(ch10 회수)·**미들웨어 체인**·**JSON 직렬화**를 채운다. ch18은 라우팅을 모른다 — 테스트는 echo/fixed 람다를 주입했지만, ch19에선 그 자리에 진짜 디스패처가 들어간다. `HttpServer`·`ConnectionLoop`는 한 줄도 안 바뀌고 핸들러만 똑똑해진다 — 그게 이 단원이 그은 경계의 값어치다.
