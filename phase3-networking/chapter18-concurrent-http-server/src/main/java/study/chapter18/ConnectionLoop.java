...
import study.chapter17.HttpMessageWriter;
 ...
 *
 * <p>{@code java.net.Socket}을 <strong>모른다</strong> — {@link InputStream}/{@link OutputStream}만 안다(ch16
 * {@code ConnectionHandler}의 "스트림만 안다" 정신 계승). 그래서 {@code ByteArrayInputStream}/{@code ByteArrayOutputStream}으로
 * 100% 결정적으로 테스트된다(소켓 0·스레드 0·sleep 0·flaky 0). 단, ch16 {@code ConnectionHandler}를
...
public final class ConnectionLoop {
     ...
     * {@code Connection} 헤더를 박은 뒤 ch17 {@link HttpMessageWriter#writeResponse}로 직렬화한다. keep-alive가 아니면
     * 그 요청 후 종료.
     *
    ...
    public static int serve(InputStream in, OutputStream out, HttpHandler handler, ServerLimits limits)
        ...
        throw new UnsupportedOperationException(
                "TODO: 루프 — parseOrNull==null이면 break(정상 종료). handler.handle→shouldKeepAlive→applyConnectionHeader→writeResponse(ch17). keepAlive 아니면 break. 처리 수 반환. 프로토콜 예외는 errorResponse로 1회 응답 후 종료(소켓 close는 호출자가)");
    }
