 ...
 *
 * <p>ch16 {@code ConnectionHandler}(스트림 단발 대화)의 HTTP판이다. 서버 엔진({@link HttpServer}·
 * {@link ConnectionLoop})은 이 함수를 <strong>받아서 구동만</strong> 한다 — "무엇을 응답할지"는 모른다.
 *
 * <p><strong>ch19의 봉합선</strong>: ch19 mini-web-framework가 이 자리에 @Route 리플렉션 라우터·미들웨어 체인·
 * JSON 직렬화를 채운다. ch18은 라우팅을 모른다 — 테스트는 echo/fixed 같은 람다를 주입한다.
 *
