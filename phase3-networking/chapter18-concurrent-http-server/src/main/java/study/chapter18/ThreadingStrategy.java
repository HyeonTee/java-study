 ...
 *
 * <p>스레드풀·가상스레드는 ch12/ch14에서 <strong>이미 직접 만들었다</strong>. 여기선 다시 만들지 않고 <strong>선택</strong>을
...
public final class ThreadingStrategy {
    ...
    /**
     * <strong>고정 스레드풀</strong>(ch12 회수) — 워커 {@code nThreads}개 재사용, 연결 &gt; 워커면 큐 대기.
     *
     * <p>힌트: 표준 {@code Executors.newFixedThreadPool(nThreads)} 반환(ch12에서 직접 만든 {@code MyThreadPool}의
     * 표준판 — 여기선 만들지 않고 선택). 블로킹 IO에서 모든 워커가 묶이면 풀 고갈이 일어남(README 진화표).
     */
    public static ExecutorService fixedPool(int nThreads) {
        throw new UnsupportedOperationException("TODO: Executors.newFixedThreadPool(nThreads) 반환(ch12 풀의 표준판)");
    }

    /**
     * <strong>연결당 가상 스레드</strong>(ch14 회수) — 풀링 안 함(가상 스레드는 싸다). IO 바운드 연결에 최적이고,
     * keep-alive의 긴 블로킹을 공짜로 흡수한다(고정 풀은 워커가 묶여 고갈).
     *
     * <p>힌트: {@code Executors.newVirtualThreadPerTaskExecutor()} 반환(ch14 회수 — <strong>풀링 금지</strong>가 핵심).
