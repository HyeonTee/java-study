...
import study.chapter17.HttpParser;
...
/**
 * 통합 테스트의 짝(완성 제공 픽스처) — ch17 {@code MiniHttpClient}(1왕복·half-close)의 <strong>keep-alive 진화형</strong>.
 *
 ...
 *
 * <p>ch17 {@code HttpMessageWriter}/{@code HttpParser}를 그대로 쓰므로, ch17을 풀기 전(main)에는 이 메서드들도 ch17
...
final class KeepAliveClient implements AutoCloseable {
    ...
    HttpResponse exchange(HttpRequest req) throws IOException {
        HttpMessageWriter.writeRequest(out, req);   // ch17 — flush 포함
        return HttpParser.parseResponse(in);        // ch17 — 같은 소켓에서 다음 응답을 이어 읽음
    }
