...
/**
 * 미니 HTTP 클라이언트 — 클라이언트↔서버 <strong>대칭</strong>의 회수(ch17 {@code EchoClient}↔{@code EchoServer}).
 *
 ...
 *
 * <p>실소켓 1왕복만 한다(ch17 4규율: loopback + {@code shutdownOutput} half-close). <strong>연결 재사용·keep-alive는
 * ch19</strong> — 여기서는 요청 하나 보내고 응답 하나 받으면 끝이다.
 */
public final class MiniHttpClient {
     ...
     * {@link HttpMessageWriter#writeRequest}로 요청 전송 → {@code socket.shutdownOutput()}으로 half-close(ch17
    ...
    public HttpResponse exchange(HttpRequest req) throws IOException {
        throw new UnsupportedOperationException(
                "TODO: Socket 연결 → writeRequest → shutdownOutput()(half-close) → parseResponse 반환. try-with-resources close. keep-alive는 ch19");
    }
}
