# Chapter 18 — 미니 웹 프레임워크 (제네릭 Handler × 라우터 × 미들웨어 양파 × JSON 직렬화 · ch17 봉합선)

> **선행 단원**: Chapter 17(concurrent-http-server — `HttpHandler` 봉합선·`Responses`), Chapter 16(http-protocol — `HttpRequest`/`HttpResponse`/`Headers`), Chapter 09(reflection-annotations — `@Route`·`RouteScanner`·`Reflect.invoke`/`getCause`), Chapter 06(record/sealed/pattern matching), Chapter 04(함수형 인터페이스·합성), Chapter 02(맵), Chapter 01·03(제네릭·바운드). **Phase 3의 마지막 단원이자 저장소의 캡스톤.**
...
> ⚠️ **이 단원은 ch17·ch16을 Gradle 의존으로 실제 재사용한다**(`build.gradle`에 `implementation project(':chapter17-...')` + `':chapter16-...')`). 봉합선을 문서가 아니라 **진짜 타입**으로 증명한다 — {@code HttpAdapter}가 진짜 `study.chapter17.HttpHandler`를 구현하고 ch16 `HttpRequest`/`HttpResponse`를 다룬다. 그 대가로 **HTTP 봉합 테스트는 `main`에서 빨간불**이다(ch16·ch17 스텁 때문). 챕터를 순서대로(ch16→ch17→ch18) 풀면 초록불이 된다. **단, 코어는 HTTP 무관 제네릭이라 ch18만 풀어도 코어 테스트는 초록불**이 된다(아래 "결정성" 참조).

---

## 이 단원의 큰 그림 — 프레임워크 = (Router ∘ dispatch) ∘ (Middleware 양파) ∘ (JsonValue 직렬화)

> ch09 `RouteScanner`의 Javadoc은 자신이 하는 일을 *"ch17/ch18 미니 웹 프레임워크가 부팅할 때 하는 바로 그 일"*이라 했고, ch17 `HttpHandler`의 Javadoc은 *"ch18이 이 자리에 라우터·미들웨어·JSON을 채운다"*고 약속했다. **ch18은 그 두 약속을 진짜 프레임워크로 갚는 자리다.**
...
`HttpAdapter`는 이 스택을 ch17 `HttpHandler`({@code HttpResponse handle(HttpRequest)}) 시그니처로 접는 **얇은 한 장**이다 — ch17 `HttpServer`에 그대로 주입된다(서버 코드는 한 줄도 안 바뀐다).

> **한 줄 슬로건**: 서버는 한 줄도 안 바뀌고, 핸들러만 똑똑해진다. **프레임워크는 마법이 아니라 18단원의 합성이다.**

### 패키지 구조 — import 방향이 아키텍처를 강제한다

다른 17개 단원은 단일 패키지(`study.chapterNN`)지만, ch18은 프레임워크라 **레이어를 패키지로 가른다**. 핵심은 **패키지 import 그래프 자체가 "코어는 HTTP를 모른다"를 컴파일 단위로 강제**한다는 것 — `study.chapter16`/`study.chapter17` import는 오직 `http` 패키지에만 존재한다.

```
study.chapter18
├── core/      # 제네릭 엔진 (의존 0 — ch16·17은 물론 routing/json/http도 모름)
...
└── http/      # ★봉합선 — core·routing·json + ch16·ch17 의존 (이 패키지만 HTTP를 안다)
    ├── HttpContext(ch16 HttpRequest 감쌈)   HttpResponses(ch17 Responses 재사용)
    └── HttpAdapter implements study.chapter17.HttpHandler
```

각 패키지엔 `package-info.java`로 레이어·의존 방향을 문서화했다. **의존 화살표는 한 방향**(`http → routing → core`, `http → json`)이라 순환이 없다 — `core`를 풀면 `core`만으로 컴파일·테스트되고, `http`만 ch16/ch17 스텁에 묶인다.

**다루지 않는 것(경계)**: JSON 파서(직렬화기만)·DI 컨테이너·완전한 Spring·검증/세션/쿠키·와일드카드/정규식 라우팅·content-negotiation → 이론 박스. 진짜 소켓·동시성 → ch17 소관(여기선 0).

---

## 핵심 박스 1 — `Handler<Req,Res>`는 전 계층의 공통 화폐

라우터·미들웨어·핸들러가 **전부 이 한 타입으로 합성**된다(ch01 제네릭 + ch04 함수형 인터페이스). ch17 `HttpHandler`는 사실 이 `Handler`의 특수화 — 개념상 `Handler<HttpRequest, HttpResponse>`다.
...
`Router.match`는 매칭 실패 시 `NotFoundException`, 메서드 불일치 시 `MethodNotAllowedException`을 **던진다**(404/405 같은 HTTP 숫자를 들지 않는다). 숫자 번역은 `HttpAdapter`의 일이다. ch16처럼 `method`는 `String`("405는 응답 단계의 결정"). 이 "던지는" 경계가 코어를 HTTP 무관 제네릭으로 유지하는 핵심이다.

`match`는 **두 단계**다(ch09 `RouteScanner.dispatch` 회수): ① method+path 둘 다 맞는 첫 라우트를 찾고, ② 없으면 *path만* 맞는 게 있었나로 405 vs 404를 가른다(경로 존재를 증명한 뒤 405를 던져야 클라이언트가 "GET이 아니라 POST를 쓰라"를 안다).
...
## ch09 회수 박스 — `Router.route` 수동 등록 = `@Route` 스캔이 자동화하는 그것

ch09 `RouteScanner.scan`은 이미 `@Route`를 리플렉션으로 읽어 `경로 → Method` 맵을 만든다(HTTP 무관 `Map<String,Method>`). ch18 `Router.route(method, pattern, handler)`는 같은 일을 **제네릭 `Handler` 값**으로 일급화한다(`Method` 대신 진짜 핸들러 객체). *"`route(...)` 수동 등록 = `@Route` 스캔이 자동화하는 그것"*이다 — 자동 스캔 실물은 ch09가 소유하므로 여기서 다시 만들지 않는다(`AnnotationRouter = RouteScanner + 메서드/경로변수 매칭 + 미들웨어 래핑`은 한 줄 확장, 이론 박스).

**리플렉션 실물 회수는 `JsonReflect.toJson` 한 곳**으로 단일화한다: record 컴포넌트를 `getRecordComponents()` + `component.getAccessor().invoke(obj)`로 읽는다(ch09 `Field.setAccessible` 대신 **record 접근자** — "같은 패턴의 다른 적용"). 접근자가 던진 예외는 `InvocationTargetException`으로 감싸여 나오므로 ch09 `Reflect.invoke` 정신대로 **`getCause()`로 풀어 재던진다**.

---

## 봉합선 박스 — `HttpAdapter`: 진짜 ch17 `HttpHandler`에 꽂힌다

`HttpAdapter implements study.chapter17.HttpHandler`. 코어가 완성되면 이 어댑터를 그대로 ch17 `HttpServer`·`ConnectionLoop`에 주입할 수 있다(ch17 "핸들러만 갈아끼운다" 회수). ch16 `HttpRequest`↔`HttpContext` 매핑(`method`/`path`는 `request.line()`에서)을 하고, 응답은 ch17 `Responses`(text/error) + ch18 `HttpResponses.json`(application/json)으로 만든다 — **이전 단원 구현을 그대로 재사용**.
...
5. **`JsonReflect`는 record 접근자**(`getRecordComponents`+`getAccessor().invoke`)를 쓰되, `InvocationTargetException`은 `getCause()`로 풀어 재던짐(ch09).
...
> (가장 작은 원자 → 양파 합성 → 경로 매칭 → 디스패처 → 직렬화 → 리플렉션 → 실전 미들웨어. **앞쪽 코어를 다 풀면 코어 테스트 8개가 초록불**, ch16까지 풀려 있으면 봉합 테스트까지 초록불.)

### 완성 제공(채우지 않음)
`Handler`/`Middleware`(추상 메서드·시그니처), `Routable`(F-bounded 계약), `RouteMatch`·`JsonValue`(+6 하위 record·팩토리), `NotFoundException`/`MethodNotAllowedException`, **봉합선** `HttpContext`(ch16 `HttpRequest` 감쌈)·`HttpResponses`(ch17 `Responses` 재사용)·`HttpAdapter`(ch17 `HttpHandler` 구현). 테스트 픽스처 `User`·`GreetingController`.
...
```sh
# 전체 테스트 — main에선 ch18 스텁(+봉합 테스트는 ch16/ch17 스텁)으로 빨간불. 챕터 순서대로 풀면 초록불.
./gradlew :chapter18-mini-web-framework:test

# 코어만(ch16/ch17 무관·인메모리 결정적) — 패키지로 통째 지정 가능
./gradlew :chapter18-mini-web-framework:test --tests "study.chapter18.core.*"
./gradlew :chapter18-mini-web-framework:test --tests "study.chapter18.json.*"

# 봉합선(진짜 ch17 HttpHandler·ch16 타입)
./gradlew :chapter18-mini-web-framework:test --tests "study.chapter18.http.SeamContractTest"
```
...
이 단원은 저장소의 끝이다. ch09가 부팅 때 하던 "마법"의 정체를, 우리가 18단원 내내 직접 짠 부품들로 완성했다:
...
| **ch09** 리플렉션·애너테이션 | `JsonReflect.toJson` (+ 상호참조 `RouteScanner`) |
| **ch16** HTTP 메시지 | `HttpContext`/`HttpResponses`가 감싼 `HttpRequest`/`HttpResponse` |
| **ch17** HttpHandler 봉합선 | `HttpAdapter`가 구현하는 진짜 `HttpHandler` |
