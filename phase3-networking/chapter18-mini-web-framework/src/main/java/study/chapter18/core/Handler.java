 ...
 *
 * <p><strong>ch17 봉합선</strong>: ch17 {@code study.chapter17.HttpHandler}({@code HttpResponse handle(HttpRequest)})는
 * 사실 이 {@code Handler}의 특수화 — 개념상 {@code Handler<HttpRequest, HttpResponse>}다. ch17은 그 자리에 echo
 * 람다를 꽂아 테스트했고, ch18은 진짜 라우터/미들웨어/직렬화를 그 자리에 채운다({@code http.HttpAdapter} 참조).
 *
