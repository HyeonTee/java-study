...
import study.chapter18.routing.MethodNotAllowedException;
...
/**
 * ★ <strong>봉합선</strong> — ch18 제네릭 코어를 ch17 {@link HttpHandler} 자리에 꽂는 얇은 어댑터. <strong>완성 제공</strong>
 * (채우지 않는다).
 *
 * <p>이게 ch17이 코드로 한 약속("ch18이 이 자리를 채운다")을 <strong>진짜 타입으로</strong> 갚는 지점이다. ch17
 * {@code HttpHandler}({@code HttpResponse handle(HttpRequest)})를 직접 {@code implements}하므로, 코어가 완성되면 이
 * 어댑터를 그대로 ch17 {@code HttpServer}·{@code ConnectionLoop}에 주입할 수 있다(서버 코드는 한 줄도 안 바뀐다 —
 * ch17 "핸들러만 갈아끼운다" 회수).
 *
 ...
 *
 * <p><strong>왜 이 단원이 ch17을 진짜 의존하는가</strong>: ch16/ch17을 챕터 순서대로 풀어왔으니, 봉합선을 문서가 아니라
 * 실제 ch17 {@code HttpHandler}·ch16 {@code HttpRequest}/{@code HttpResponse}로 증명한다(이전 단원 구현 재사용). 그 대가로
 * main에선 ch16·ch17 스텁 때문에 봉합 테스트가 빨간불이지만, 순서대로 풀면 초록불이 된다.
 */
