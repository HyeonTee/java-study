...
/**
 * 임의 객체(특히 {@code record} 컴포넌트) → {@link JsonValue} 매핑 — ch09 {@code ObjectMapper.toMap}의 JSON판
 * (<strong>리플렉션 회수</strong>). {@link JsonWriter}(직렬화)와 분리해 분량·결합을 통제한다.
 *
 * <p><strong>리플렉션 실물 회수는 이 한 곳</strong>으로 단일화한다(ch09 {@code RouteScanner}와 중복 회피). ch09가
...
public final class JsonReflect {
     ...
     * </ul>
     *
     * <p><strong>함정(ch09 회수)</strong>: record 컴포넌트는 {@code type.getRecordComponents()}로 얻고, 값은
     * {@code component.getAccessor().invoke(obj)}로 읽는다(필드 {@code setAccessible} 대신 <strong>접근자 메서드</strong>).
     * 접근자가 내부에서 예외를 던지면 {@code InvocationTargetException}으로 감싸여 나오므로, ch09 {@code Reflect.invoke}
