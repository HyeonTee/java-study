...
import study.chapter18.core.Middlewares;
...
/**
 * ★ <strong>캡스톤 통합</strong> — 라우터 + 미들웨어 + JSON 직렬화 + ch17 봉합선이 한 흐름으로. <strong>코어만 채우면
 * 저절로 초록불</strong>이 되는 마무리 카타르시스("프레임워크는 18단원의 합성이다").
 *
 * <p>main에선 ch16/ch17 스텁 때문에 빨간불 — 챕터 순서대로 풀면 GET /users/{id}가 JSON·200·경로변수까지 끝까지 흐른다.
 */
@DisplayName("프레임워크 통합 — 라우터×미들웨어×JSON×ch17 봉합선 (전 계층)")
