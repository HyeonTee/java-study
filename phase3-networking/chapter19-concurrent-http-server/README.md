...
> 🔧 **빨간불이면 이 순서로 확인** — "내 코드가 틀렸나, 의존 단원을 안 풀었나"를 가르는 체크리스트:
> 1. `main` 브랜치인가? → ch19가 전부 red인 게 **정상**이다(아직 아무것도 안 풀었으니). 여기선 내 코드를 의심할 필요 없다.
> 2. `solve`인데 red인가? → **ch18을 먼저 초록불**로 만들었는지 확인하라(`./gradlew :chapter18-http-protocol:test`). ch19는 ch18 구현 위에서만 통과한다.
> 3. **ch18이 green인데도 ch19가 red**라면 — 그때가 비로소 *내 ch19 코드*를 의심할 때다.
