...
/**
 * ch18 파서를 <strong>한계 강제(DoS 방어) + keep-alive 종료 신호</strong> 버전으로 감싸는 데코레이션.
 * (ch18 {@code HttpParser}는 손대지 않는다 — 그 위에 한 겹.)
 *
 * <p><strong>이 단원의 선행-연결 핵심</strong>: ch18 {@code HttpParser.parse}는 즉시 EOF면 {@link HttpProtocolException}을
 ...
 * {@code null}</strong>을 반환한다. <strong>ch18 {@code parse}를 keep-alive 루프에 그대로 쓰면 안 된다</strong>(idle
...
public final class BoundedHttpParser {
    ...
    /**
     * ch18 {@code HttpParser.readLine}과 같은 계약(한 바이트씩 {@code '\n'}까지, 끝 {@code '\r'} 제거, UTF-8, 즉시
     ...
     *
     * <p>힌트: ch18 readLine 로직(풀이 재사용 가능)에 바이트 카운터를 더해, 카운터가 {@code maxBytes}를 넘으면
    ...
    public static String readLineBounded(InputStream in, int maxBytes) throws IOException {
        throw new UnsupportedOperationException(
                "TODO: read() 한 바이트씩 누적(ch18 readLine 회수) + 카운터. maxBytes 초과 순간 즉시 RequestLimitException(431). EOF→null, 빈 줄 \"\"");
    }

    /**
     * {@link #readLineBounded}를 빈 줄({@code ""})이 나올 때까지 반복해 {@code study.chapter18.Headers}를 모은다.
     * 헤더 줄 수가 {@code limits.maxHeaderCount()}를 넘으면 {@link RequestLimitException}(431). 빈 줄 없이 EOF면
     * {@link HttpProtocolException}(잘린 헤더).
     *
     * <p>힌트: 각 줄 {@code split(":", 2)}로 name/value(value trim)는 ch18 {@code readHeaders} 로직 회수.
     ...
     */
    public static study.chapter18.Headers readHeadersBounded(InputStream in, ServerLimits limits) throws IOException {
        throw new UnsupportedOperationException(
    ...
    /**
     * 요청 한 개를 한계 안에서 파싱한다(ch18 {@code parse}의 bounded 버전).
     *
     * <p><strong>핵심 차이</strong>: 요청을 읽기 전 <strong>즉시 EOF면 {@code null}</strong>을 반환한다(keep-alive
     * 루프의 정상 종료 신호 — "다음 요청 없음"). ch18 {@code parse}는 여기서 예외를 던졌다(N왕복 계약이라 다르다).
     *
     * <p>힌트: 첫 {@code readLineBounded(maxRequestLineBytes)}가 {@code null}이면 곧장 {@code null} 반환. 아니면
     * ch18 {@code parseRequestLine} → {@code readHeadersBounded} → {@code BodyFraming.from} 분기로 바디. 바디 길이
     * ({@code Content-Length})가 {@code maxBodyBytes}를 넘으면 {@link RequestLimitException}(413). 바디는 ch18
    ...
    public static HttpRequest parseOrNull(InputStream in, ServerLimits limits) throws IOException {
        throw new UnsupportedOperationException(
                "TODO: 첫 줄이 null(요청 전 EOF)이면 null 반환(ch18 parse는 여기서 예외 — N왕복 계약). 아니면 parseRequestLine→readHeadersBounded→BodyFraming 분기. Content-Length>maxBodyBytes면 413");
    }
}
