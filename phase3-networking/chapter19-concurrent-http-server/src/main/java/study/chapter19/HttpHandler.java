 ...
 *
 * <p>ch17 {@code ConnectionHandler}(스트림 단발 대화)의 HTTP판이다. 서버 엔진({@link HttpServer}·
 * {@link ConnectionLoop})은 이 함수를 <strong>받아서 구동만</strong> 한다 — "무엇을 응답할지"는 모른다.
 *
 * <p><strong>ch20의 봉합선</strong>: ch20 mini-web-framework가 이 자리에 @Route 리플렉션 라우터·미들웨어 체인·
 * JSON 직렬화를 채운다. ch19은 라우팅을 모른다 — 테스트는 echo/fixed 같은 람다를 주입한다.
 *
 * <p>완성 제공(채우지 않는다). 핸들러는 부수효과 없는 순수 함수가 이상적이며, 공유 가변 상태를 쓰면
 * <strong>스레드 안전</strong>해야 한다(동시 연결이 같은 핸들러를 호출하므로 — ch08/ch11 회수).
 */
@FunctionalInterface
