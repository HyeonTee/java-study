 ...
 * ch17 {@code EchoServer}를 한 겹 위로 계승.
 *
 ...
 *
 * <p>비결정성 길들이기(ch17 4규율): loopback + ephemeral port({@code :0}) + accept 소켓 {@code setSoTimeout} +
 * accept 디스패처는 <strong>데몬 스레드</strong>. {@code AutoCloseable}이다.
 */
public final class HttpServer implements AutoCloseable {

    /**
     * {@code ServerSocket}을 loopback의 ephemeral port({@code :0})에 bind한다(ch17 {@code EchoServer} 회수).
     *
     ...
     * <strong>정지 플래그는 반드시 {@code volatile}</strong>(accept 스레드와 {@link #close} 스레드 사이 가시성 — ch08/ch11 회수).
     *
     ...
     * 루프를 빠져나간다(ch17 회수). {@code closed}는 volatile.
     */
    public void start() {
        throw new UnsupportedOperationException(
     ...
     * 한 연결의 예외는 <strong>그 연결만 격리</strong>한다(소켓만 닫고 다음 연결·서버 전체엔 영향 0 — ch13 "작업 하나
     * 실패가 풀을 안 죽임" 회수).
     *
     ...
     * {@code SocketException}으로 깨움, ch17 회수). 그다음 주입 {@link Executor}를 graceful하게 정리한다.
     *
