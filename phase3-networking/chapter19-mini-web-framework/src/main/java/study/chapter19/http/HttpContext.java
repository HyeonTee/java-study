...
/**
 * ch17 {@link HttpRequest}를 <strong>그대로 감싸고</strong> 추출된 경로변수를 곁들인 라우팅 컨텍스트 — {@code routing.Router}의
 * {@code Req}로 쓰인다. <strong>완성 제공</strong>.
 *
 * <p><strong>이전 단원 재사용</strong>: 별도 요청 모델을 새로 만들지 않고 진짜 ch17 {@link HttpRequest}(요청라인·헤더·바디)를
