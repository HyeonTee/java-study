...
import study.chapter19.json.JsonWriter;
import study.chapter17.Headers;
import study.chapter17.HttpResponse;
import study.chapter17.StatusLine;
import study.chapter18.Responses;

import java.nio.charset.StandardCharsets;

/**
 * ch17 {@link HttpResponse} 빌더 — 프레임워크가 응답을 만드는 자리. <strong>완성 제공</strong>.
 *
 * <p><strong>이전 단원 재사용</strong>: 텍스트·에러 응답은 ch18 {@link Responses}(ok/text/error, {@code Content-Length}
 * 자동 확정)에 그대로 위임한다. JSON 응답만 ch19이 더한다({@code application/json} + {@link JsonWriter}). 즉 ch17
 * {@link Headers}/{@link StatusLine}·ch18 {@link Responses}를 회수해, ch19은 "JSON 한 겹"만 얹는다.
 *
 * <p>ch17을 풀기 전(main)에는 ch18 {@code Responses}·ch17 {@code Headers.with}가 스텁이라 이 메서드들도
 * {@code UnsupportedOperationException}을 던진다 — 의존 단원 스텁 때문. 챕터 순서대로 ch17까지 풀면 동작한다.
 */
