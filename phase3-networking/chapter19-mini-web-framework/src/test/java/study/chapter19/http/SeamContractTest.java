...
/**
 * ★ <strong>봉합선 계약</strong> — {@link HttpAdapter}가 진짜 ch18 {@link HttpHandler}로서 ch17
 * {@link HttpRequest}→{@link HttpResponse}를 처리한다. status 번역(404/405/500)을 검증.
 *
 * <p><strong>main에선 빨간불</strong>: ch17 {@code Headers.with/get}·ch18 {@code Responses}·ch19 {@code Router} 스텁
 * 때문(챕터 순서대로 ch17·ch18·ch19을 풀면 초록불). 단, 소켓은 없다 — {@code HttpHandler}는 순수 함수라 100% 인메모리.
 */
@DisplayName("봉합선 — HttpAdapter는 진짜 ch18 HttpHandler (404/405/500 번역)")
class SeamContractTest {
    ...
    void 어댑터는_ch17_HttpHandler로서_JSON을_응답() {
        HttpHandler handler = new HttpAdapter(userRouter());   // IS-A 진짜 ch18 HttpHandler
