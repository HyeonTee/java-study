...
import study.chapter20.core.Routable;

/**
 * 메서드 + 경로패턴 매칭으로 {@link Handler}를 고르는 코어 디스패처 — <strong>이 단원의 간판</strong>.
 * ch02 라우트 테이블 + ch01 제네릭 + {@link PathPattern} 위에 선다.
 *
 * <p><strong>ch10 회수</strong>: ch10 {@code RouteScanner.scan}은 {@code @Route}를 리플렉션으로 읽어 {@code 경로 → Method}
 ...
 * 실물은 ch10가 소유하므로 여기서 다시 만들지 않는다(README 상호참조 박스).
 *
 ...
 * 숫자 번역은 {@code http.HttpAdapter}의 일이다. {@code method}는 ch18처럼 {@code String}(enum으로 닫지 않음 — "405는 응답
 * 단계의 결정").
 *
...
public final class Router<Req extends Routable<Req>, Res> {
     ...
     *
     * <p>힌트(ch10 {@code RouteScanner.dispatch} 회수): 두 단계로 — (1) method+path 둘 다 맞는 첫 라우트를 찾고, 있으면
    ...
    /**
     * 라우터 전체를 <strong>미들웨어로 감싸인 단일 {@link Handler}</strong>로 닫는다 — 프레임워크가 ch19
