...
import study.chapter20.json.JsonReflect;
import study.chapter20.json.User;
...
/**
 * ★ <strong>봉합선 계약</strong> — {@link HttpAdapter}가 진짜 ch19 {@link HttpHandler}로서 ch18
 * {@link HttpRequest}→{@link HttpResponse}를 처리한다. status 번역(404/405/500)을 검증.
 *
 * <p><strong>main에선 빨간불</strong>: ch18 {@code Headers.with/get}·ch19 {@code Responses}·ch20 {@code Router} 스텁
 * 때문(챕터 순서대로 ch18·ch19·ch20을 풀면 초록불). 단, 소켓은 없다 — {@code HttpHandler}는 순수 함수라 100% 인메모리.
 */
@DisplayName("봉합선 — HttpAdapter는 진짜 ch19 HttpHandler (404/405/500 번역)")
